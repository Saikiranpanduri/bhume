...
def optimize_plot(poly_raster, boundary_img, transform):
    """
    Optimize plot position through translation search (FAST VERSION).
    
    ±15 pixels with 1-pixel steps = 961 candidates (8× original, but much faster than full version)
    No rotation to keep speed acceptable.
    """
    best_score = -1.0
    best_poly = poly_raster

    # Balance: ±15px, 1px steps = 31×31 = 961 candidates
    # ~3× slower than original (121 candidates) but still fast
